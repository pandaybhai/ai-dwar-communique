-- Retention purge extended to Shopify commerce data.
-- Deletes orders/order_items/abandoned_checkouts older than 24 months and
-- strips personal fields from the raw payload of anything older than 90 days.
-- Contact logic is unchanged.
  ...
  WITH u AS (
    UPDATE public.orders
     ...
     WHERE created_at < now() - interval '90 days'
       AND (raw ? 'customer' OR raw ? 'billing_address' OR raw ? 'email')
    RETURNING 1
  ...
  WITH u AS (
    UPDATE public.abandoned_checkouts
       SET raw = raw - 'customer' - 'billing_address' - 'shipping_address'
                     - 'shipping_lines' - 'email' - 'phone'
     WHERE created_at < now() - interval '90 days'
       AND (raw ? 'customer' OR raw ? 'billing_address' OR raw ? 'email' OR raw ? 'phone')
    RETURNING 1
